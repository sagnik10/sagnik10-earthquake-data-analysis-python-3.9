...
class Context:
    ...
    def identify(self, csi_type, ops_type, csi_name, group=None)->int:
        if ops_type not in self._csi_names[csi_type]:
            self._csi_names[csi_type][ops_type] = {}
            return None
        
        if group is not None and \
            f"{group}/{csi_name}" in self._csi_names[csi_type][ops_type]:
            return self._csi_names[csi_type][ops_type][f"{group}/{csi_name}"]
    ...
    def define(self, csi_type, ops_type, csi_name=None, item=None, group=None)->int:
        if ops_type not in self._csi_names[csi_type]:
            self._csi_names[csi_type][ops_type] = {}
