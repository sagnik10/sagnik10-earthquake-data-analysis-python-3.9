...
from shps.plane.shape import _simplify

ccode = lambda *args: _ccode(*args, standard='C89')

def stringify(basis, keys=None, symbolic=False, latex=True):
    """
    Eventually to replace shps.plane._stringify.
    """
    A    = basis.vandermonde()
    func = lambda x,y: [eval(t,dict(x=x,y=y)) for t in basis.terms]

    if keys is None:
        keys = list(basis.nodes.keys())
